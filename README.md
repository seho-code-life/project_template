...
关于eslint的校验，模板中内置了，但是typescript的内容却没有校验，你可以使用tsc或者vue-tsc (模板中已经安装)，关于ts的校验，可能之后会默认支持，但是目前因为业务情况，暂时不需要强制的代码提交前的ts校验。

```
tsc --noEmit 或者 vue-tsc --noEmit
```

vue-tsc 是基于Volar的ts校验，开发者可以选择在vscode使用volar进行校验，这也是一个替代方案。
